           ...
           int R3[3])
{
    ...
    this->route_channel(0, route_table[0][0], route_table[0][1]);
    this->route_channel(1, route_table[1][0], route_table[1][1]);
    this->route_channel(2, route_table[2][0], route_table[2][1]);
    ...
    LOG("\t>            |   Ch1   |   Ch2   |   Ch3   |\n");
    LOG("\t>            |%sabled |%sabled |%sabled |\n",
        chx_enable[0] ? " en" : "dis", chx_enable[1] ? " en" : "dis",
        chx_enable[2] ? " en" : "dis");
    LOG("\t> Route      |(-)%d (+)%d|(-)%d (+)%d|(-)%d (+)%d|\n",
        route_table[0][0], route_table[0][1], route_table[1][0],
        route_table[1][1], route_table[2][0], route_table[2][1]);

    LOG("\t> Resolution |  %s   |  %s   |  %s   |\n",
...
EMG::route_channel(uint8_t channel, uint8_t pos_in, uint8_t neg_in)
{
    ...
    m_master->writeReg(m_id, FLEX_CH0_CN_REG + channel, val);
}
...
void
EMG::enable_channels(bool ch0, bool ch1, bool ch2)
{
    uint8_t val =
...
EMG::config_R1(uint8_t R1_ch0, uint8_t R1_ch1, uint8_t R1_ch2)
{
    ...
    uint8_t val = R1_ch0 | R1_ch1 | R1_ch1;
...
EMG::config_R3(int ch, uint8_t R3)
{
    ...
    m_master->writeReg(m_id, R3_RATE_CH0_REG + ch, R3);
}
